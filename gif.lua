...
local function open_file_buffer(filename)    -- open file for read-only, returns InputBufferObject
   ...
   local file = assert(fileOpen(filename, true))
   local file_size = assert(fileGetSize(file))
   ...
   local function get_file_block(block_index)
      ...
      if not object then
         ...
         assert(fileSetPos(file, block_offset))
         local content = fileRead(file, block_length)
...
function dxCreateGIF(path, format, edge)
    local gif = assert(open_gif(path), "Failed to open GIF file")
    local parameters = gif.get_file_parameters()
    local object = {}
...
_dxGetMaterialSize = dxGetMaterialSize
function dxGetMaterialSize(element)
    if type(element) == "table" and element.element and element.element == "GIF" then
        return element.width, element.height
    end

    return _dxGetMaterialSize(element)
end

function getGIFFrames(element)
    assert(element and type(element) == "table" and element.element and element.element == "GIF", "Bad argument @ 'getGIFFrames' [Expected GIF element at argument 1, got "..type(element).."]")
...
function dxDrawImage(x, y, w, h, image, delay, ...)
    if type(image) == "table" and image.element and image.element == "GIF" then
        if image.update < getTickCount() then
            image.frame = image.frame + 1
            if image.frame > image.frames_count then
                image.frame = 1
            end
            image.update = getTickCount() + delay
        end
